...
/* Temporary hack until GNU Make can be changed to use the
...
/* Check if vaxc$path exists */
static int check_vaxc_path(void) {
    const char * logname = "VAXC$PATH";
...
/* How to simply create vaxc$path */
static int make_vaxc_path(void) {
    int ret_val;
    const char * proc_table = "LNM$PROCESS_TABLE";
    const char * logname = "VAXC$PATH";
    const char * path1 = "GNV$GNU:[usr.local.bin]";
    const char * path2 = "GNV$GNU:[usr.local.sbin]";
    const char * path3 = "GNV$GNU:[usr.bin]";
    const char * path4 = "GNV$GNU:[usr.sbin]";
    const char * path5 = "GNV$GNU:[bin]";
    const char * path6 = "GNV$GNU:[sbin]";
    const char * path7 = "SYS$DISK:[]";

    struct dsc$descriptor_s proc_table_dsc;
    struct dsc$descriptor_s logname_dsc;
    struct itmlst_3 item_list[8];
    ...
    item_list[0].buflen = strlen(path1);
    item_list[0].itmcode = LNM$_STRING;
    item_list[0].bufadr = (char *)path1;
    item_list[0].retlen = NULL;

    item_list[1].buflen = strlen(path2);
    item_list[1].itmcode = LNM$_STRING;
    item_list[1].bufadr = (char *)path2;
    item_list[1].retlen = NULL;

    item_list[2].buflen = strlen(path3);
    item_list[2].itmcode = LNM$_STRING;
    item_list[2].bufadr = (char *)path3;
    item_list[2].retlen = NULL;

    item_list[3].buflen = strlen(path4);
    item_list[3].itmcode = LNM$_STRING;
    item_list[3].bufadr = (char *)path4;
    item_list[3].retlen = NULL;

    item_list[4].buflen = strlen(path5);
    item_list[4].itmcode = LNM$_STRING;
    item_list[4].bufadr = (char *)path5;
    item_list[4].retlen = NULL;

    item_list[5].buflen = strlen(path6);
    item_list[5].itmcode = LNM$_STRING;
    item_list[5].bufadr = (char *)path6;
    item_list[5].retlen = NULL;

    item_list[6].buflen = strlen(path7);
    item_list[6].itmcode = LNM$_STRING;
    item_list[6].bufadr = (char *)path7;
    item_list[6].retlen = NULL;

    item_list[7].buflen = 0;
    item_list[7].itmcode = 0;
...
#ifdef __VMS
    /* If VAXC$PATH does not exist, create one */
    if (check_vaxc_path()) {
	make_vaxc_path();
    }
#endif
...
# endif
  ...
  {
    struct variable *shv;
    shv = lookup_variable ("GNV_MAKE_DEBUG", 14);
    if (shv) {
      if (shv->value && (strcmp(shv->value,"0")!=0))
	gnv_debug_make = 1;
    }
  }
#endif
  ...
  /* We need to know what kind of shell we will be using.  */
  {
    char *shell_env;
    /*
     * Default to using DCL
     */
    unixy_shell = 0;

    /*
     *  If shell variable is defined to be something Unixy, then use it
     */
    shell_env = getenv("SHELL");
    if (shell_env) {
      char *shell=basename(shell_env);

      if ((strcasecmp(shell,"sh.exe")==0) ||
	  (strcasecmp(shell,"sh")==0) ||
	  (strcasecmp(shell,"bash.exe")==0) ||
	  (strcasecmp(shell,"bash")==0))
	unixy_shell = 1;
    }
  }
#endif
#if defined(VMS)
    /*
     *  User can pick version using UNIX_MODE
     */
    {
      struct variable *shv;
      shv = lookup_variable ("UNIX_MODE", 9);
      if (shv) {
	if (shv->value && (strcmp(shv->value,"0")!=0))
	  unixy_shell = 1;
	else
...
print_version ()
{
  ...
  printf ("%sGNV Fork of GNU Make version %s", precede, version_string);
...
%sPARTICULAR PURPOSE.\n\n\
%sReport bugs to https://sourceforge.net/p/gnv/bugs/\n\n"),
          precede, make_host,
