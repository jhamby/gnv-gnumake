...
#if defined(VMS)
  if (!unixy_shell) {
    if (((exit_code>=2) && (exit_code<=255)) || !(exit_code & 1))
     ...
     register struct child *child;
{
...

  {
    char *end = 0;
#ifdef VMS
    /* Just construct an argv to make us look like everybody else */
    if (!unixy_shell) {
...
#ifdef VMS
      /*
       * If this is not a unixy_shell, skip this test
       */
      unixy_shell &&
#endif
#ifdef __MSDOS__
     ...
     struct child *child;
{
    ...
    if (make_print_exec == -1) {
	if (getenv("MAKE_PRINT_EXEC"))
	    make_print_exec = 1;
	else
	    make_print_exec = 0;
    }
/*
 *  Fixup commonly used names for bash
 */
 ...
 */
	...
	printf("writing long argument to file: %s\n",argv[2]);
      ...
      if ((temp_handle=mkstemp(temp_name))==-1) {
	perror("mkstemp"); exit(1);
      ...
      new_argv[0]=argv[0];
      new_argv[1]=temp_name;
      new_argv[2]=NULL;

      if (make_print_exec) {
	printf("DEBUG: a long shell command\n");
	printf("DEBUG: %s\n",argv[2]);
      }
      pid=vfork();
      if (pid == -1) {
	...
	return -1;
      ...
      char **p=argv;
      if (make_print_exec) {
	printf("DEBUG: not a long shell command\n");
	printf("DEBUG:");
      }
      while (*p!=NULL) {
        if (make_print_exec)
	...
	*/
	if ((errno == ENOEXEC) && (strcasecmp(argv[0],default_shell) != 0)) {
	    char **new_argv;
	    int argc;
...
#ifdef VMS
  /*
  ** exec_command is only used on OpenVMS by the re_exec code in main.c. In
  ** this case we are called without a fork because what is required is a
  ** restart of the program. Calling exec without a matching vfork is very
  ** much a no-no on OpenVMS. So instead we re-run the image as a child,
  ** capture its exit status, and then return that ourselves to our parrent.
  */
  int pid, p, status=0;
  ...
  if (pid == -1) {
    /* error */
    perror_with_name ("vfork(ec): ", argv[0]);
    _exit (EXIT_FAILURE);
  }
  if (pid == 0) {
    /* child */
    execve(argv[0], argv, envp);
    perror_with_name ("execve(ec): ", argv[0]);
    _exit (EXIT_FAILURE);
  }
  ...
  if (p != pid) {
    perror_with_name ("waitpid(ec): ", argv[0]);
    _exit (EXIT_FAILURE);
  }

  /* mimic child's exit status/signal */
...
#if !defined(VMS) || 1
/*
 * This is needed on VMS if we are envoked in UNIX mode.
 */
