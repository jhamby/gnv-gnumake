...
#ifdef VMS
#define IS_PATHSEP(c) (unixy_shell ? ((c) == '/') : ((c) == ']'))
#else
...
#ifdef VMS
	    o = variable_buffer_output (o, unixy_shell ? "./" : "[]", 2);
#else
#ifndef _AMIGA
     ...
     const char *funcname;
{
...
#ifdef VMS
  static int init_done=0, nullfd, make_pipe_size;
  char *p;
#endif
  ...
  if (!init_done) {
    ...
    if (nullfd == -1) {
      perror_with_name ("open"," /nla0");
      return o;
    }
    p = getenv("MAKE_PIPE_SIZE");
    if (p)
      make_pipe_size = atoi(p);
    else
      make_pipe_size = 4096;
    init_done = 1;
  }
...
# else
#ifdef VMS
  if (decc$pipe (pipedes,0,make_pipe_size) < 0)
#else
  if (pipe (pipedes) < 0)
#endif
    {
...
# endif /* ! __MSDOS__ */
	...
	{
#ifdef VMS
          static int init_done=0, do_debug;
#endif
          if (i == maxlen)
	    {
	      maxlen += 512;
...
#ifdef VMS
	  /*
	  ** This next bit is funky. We sometimes get back a zero length
	  ** read when there is STILL MORE data in the pipe. This seems to
	  ** happen with constructs such as $(shell ls -ls; pwd). Originally
	  ** I had a fix in here which would just wait for two consecutive
	  ** zero length reads, and that worked pretty well, until I started
	  ** doing long Mozilla builds. Then I found that very occasionally
	  ** make would hang, and upon further debugging I found that it was
	  ** hung waiting for the second zero length read. So the heuristic
	  ** was modified to only look for a second zero length read if we'd
	  ** has at least one full pipe read. This seems to work much better.
	  ** At least until the next problem.
          **
          ** 6/24/03 - well, it happened. By not always waiting for the second
          ** consecutive zero length read, I found a situation where the above
          ** would fail:
          **
          **   X = $(shell for x in a b c; do /dka0/test/test; done)
          **
          ** So I've reinstated the original code (which waits for two
          ** consecutive zero length reads before quitting), and tried to
          ** reproduce the hang problem, but I can't. Maybe the hang was
          ** caused by some other problem which has since been fixed? Who
          ** knows. But for now I need to leave this code in. But in case
          ** the hang should ever come back (or any other situation which
          ** causes this code to break again), I've added some debug code:
          ** just define MAKE_DEBUG_SHELL_READS (to anything).
	  **
	  ** 6/7/05 - Ah... Its happened again.  Its in the middle of a great
	  ** big build, and I don't have the line that's doing it, but it
	  ** has happened.  The interesting bit is, we studied it in SDA,
	  ** and determined that this mailbox (pipe) has a reference count
	  ** of one... implying that we're reading it, but there's no
	  ** writer.  I don't know what's happened to the writer, but this
	  ** does give me something to check.
	  **
	  ** Don't do the second read if there's no second channel to the
	  ** pipe.
          */
          if (!init_done) {
            do_debug = getenv("MAKE_DEBUG_SHELL_READS") != NULL;
	    init_done = 1;
          }
          if (do_debug) {
            printf("read1 returned %d",cc);
            if (cc > 0) {
                buffer[i+cc] = '\0';
                printf(": %s",&buffer[i]);
            }
...
#if 0
          if ((cc == 0) && (errno == 0) && (maxlen != 200)) {
	      cc = read (pipedes[0], &buffer[i], maxlen - i);
	  }
#else
          if ((cc == 0) && (errno == 0) )
	     {
	      int pipe_has_two_channels (int);

	      // Only do the second read, if there's another channel
	      // still open on the pipe.
	      if (pipe_has_two_channels (pipedes [0]))
		{
		 cc = read (pipedes[0], &buffer[i], maxlen - i);
                 if (do_debug) {
                	printf("read2 returned %d",cc);
                	if (cc > 0) {
                    	  buffer[i+cc] = '\0';
                    	  printf(": %s",&buffer[i]);
                	  }
...
if (fp) fname = fgetname (fp, buff, 1);
...
/* pipe_has_two_channels
 *
 * This function is very VMS specific.
 * We seem to have a problem where the write end of a pipe is going away.
 * So we need to detect how many channels are still on this mailbox.
 * This routine checks the number of channels.  If its not at least two,
 * then there's trouble.
 *
 * Return 1 if there's two (or more) channels assigned
 * Return 0 if there are less than two channels assigned
 */

int pipe_has_two_channels (int pipe_des)
{
int status;
int iosb[2];
int refcnt, retlen;
char *fname = 0;
struct dsc$descriptor_s fn_descript = {0, DSC$K_DTYPE_T, DSC$K_CLASS_S, 0};
struct itmlst
   {
    short int len;
    short int code;
...
struct itmlst dvi_itm = {4, DVI$_REFCNT, &refcnt, &retlen };

// If its not a pipe... then bail out!
if (!isapipe (pipe_des)) return 0;

// First, find the filename... For a pipe, its the mailbox device.
fname = get_fd_name (pipe_des);

// If that failed, bail out!
if (!fname) return 0;

// Now for the fun part.  SYS$GETDVI gives us the reference count of the device.
// Need a descriptor for the device name.
fn_descript.dsc$a_pointer = fname;
fn_descript.dsc$w_length = strlen (fname);

status = sys$getdviw (0, 0, &fn_descript, &dvi_itm, iosb, 0, 0, 0, 0);
if (!(status & 1) || iosb [0] != 1) return 0;
